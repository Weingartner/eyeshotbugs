...
namespace EyeshotBugs
{
        public class ToolBodyCylinderWithInsertRadius 
    {
        ...
        public ToolBodyCylinderWithInsertRadius(double length, double radius, double insertRadius)
        {
            ...
            SurfaceOfRevolutionAxis = new Line(p0, p3);

            ToolBody = SurfaceOfRevolutionCrossSection.RevolveAsSolid3D(0, System.Math.PI * 2, SurfaceOfRevolutionAxis);
        }

        public Solid3D ToolBody { get; }
        public ICurve SurfaceOfRevolutionCrossSection { get; }
        public Line SurfaceOfRevolutionAxis { get; }
    }
        ...
        public async Task Solid3DIntersectIsWonky()
        {
           ...
           while (!closed)

            {
                ...
                Eyeshot.ViewportLayout.ViewportLayout.Invalidate();

                await Task.Delay(TimeSpan.FromSeconds(0.5));

                var intersection = Solid3D.Intersection(stockSolid, tbSolid);

                Eyeshot.ViewportLayout.ViewportLayout.Entities.Remove(tbSolid);
                Eyeshot.ViewportLayout.ViewportLayout.Entities.Remove(stockSolid);
